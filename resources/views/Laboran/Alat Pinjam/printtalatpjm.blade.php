...
<head>
    ...
    <title>Print Pemakaian Alat</title>
</head>
...
@php
    ...
    // Ambil tanggal peminjaman dari data pertama (diasumsikan sama untuk semua dalam grup)
    $tanggalPeminjaman = $alatpjm->where('status', 'Disetujui')->first()->tanggal_peminjaman ?? null;
@endphp
        ...
        <section class="section">
            <div class="row">
                <div class="col-lg-12">
                    <div class="card">
                        <div class="card-body">
                            <a href="{{ route('alatpjm') }}" class="btn btn-danger mt-3">
                            ...
                            <div class="printable-area">
                                <h5 class="mt-4">Tanggal: {{ \Carbon\Carbon::parse($tanggalPeminjaman)->isoFormat('DD MMMM YYYY') }}</h5>
                                        ...
                                        @foreach($groupedAlatpjm as $data)
                                            @if (session('level') != 'Mahasiswa' || (session('level') == 'Mahasiswa' && $data->google_id == Auth::user()->google_id))
                                                <tr>
                                                    ...
                                                    <td>
                                                        @php
                                                            $stokAlat = \App\Models\mdlAlat::where('nama_alat', $data->nama_alat)->first();
                                                        @endphp
                                                        {{ $stokAlat ? $stokAlat->stok . ' ' . $stokAlat->satuan : 'Stok tidak tersedia' }}
                                                    </td>
                                                </tr>
                                            @endif
                                        @endforeach
                                ...
                                <h6 class="mt-3">Keperluan: {{ $alatpjm->first()->keperluan ?? 'Tidak ada keperluan yang tercantum' }}</h6>
                            </div>
                        </div>
                    </div>
                </div>
